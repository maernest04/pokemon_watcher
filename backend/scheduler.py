...
def _send_listing_alert(
    ...
    if market_price is None:
        market_text = "Market price not yet available"
    elif pct_below_market is None:
        market_text = f"${market_price:.2f}"
    else:
        market_text = f"${market_price:.2f} ({pct_below_market * 100:.1f}% below market)"
    fields = [
        {"name": "Query", "value": search_query.query_string, "inline": False},
        {
            "name": "Price",
            "value": f"${price:.2f}" if price is not None else "Unknown",
            "inline": True,
        },
        {"name": "Market Price", "value": market_text, "inline": True},
        {
            "name": "Listing",
            "value": listing.get("url") or "Unavailable",
            "inline": False,
        },
    ]
    send_embed(
...
async def refresh_market_prices_job() -> None:
    ...
    for index, card_query in enumerate(unique_queries):
        try:
            result = scrape_market_price(card_query)
        except Exception:
        ...
        if market_price is not None:
            ...
            try:
                cache = db.scalar(
                    select(PriceCache).where(PriceCache.card_query == card_query)
                )
                if cache is None:
                    cache = PriceCache(card_query=card_query)
                    db.add(cache)
